...
def _run_stop(monkeypatch, capsys, tmp_path, verdict, *, session="s1", env=None):
    ...
    lines = [
        {"type": "user", "uuid": "u1",
         "message": {"content": [{"type": "text", "text": "finish everything"}]}},
...
def test_cap_alerts_instead_of_spinning(monkeypatch, capsys, tmp_path):
    ...
    # and the counter reset: the next turn blocks again
    rc, out = _run_stop(monkeypatch, capsys, tmp_path, _UNMET)
